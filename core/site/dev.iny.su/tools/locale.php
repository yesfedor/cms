...
<div class="container my-5">
    ...
    <div class="row mt-2">
        <div class="col-12 text-center">
            <button do-add onclick="" class="btn btn-outline-primary btn-rounded"></button>
            <button do-save onclick="" class="btn btn-outline-success btn-rounded"></button>
        </div>
    </div>
</div>
<script>
devLocale = {
    ...
    init() {
        ...
        $('[do-add]').html('Добавить').attr('onclick', 'devLocale.render();')
        $('[do-save]').html('Сохранить').attr('onclick', 'devLocale.save();')
    },
    render() {
        switch(devLocale.data.editable) {
            case false:
