        ...
        <div class="col-12 d-block my-5 py-3">
            <div class="row my-5">
                <div id="blogPostWrapper" class="col-12">
                    <h2 class="h2 mb-3"><?= $postData['title'] ?></h2>
                    <h4 class="mb-3"><?= textLink($postData['preview']) ?></h4>
                    ...
                    foreach($postData['content'] as $key => $val) {
                        if ($val['type'] == 'h2' or $val['type'] == 'h4' or $val['type'] == 'h5') {
                            ...
                            echo '<'.$val['type'].' class="theme-text text-'.$val['align'].' '.$valB.' my-3 mark-pre">'.textLink($val['value']).'</'.$val['type'].'>';
                        } else {
                    ...
                    ?>
                    <hr class="w-100 py-1 my-3">
                    <span class="d-inline-block w-50 float-left theme-text text-left"><?= $postData['date_create'] ?></span>
                    <span class="d-inline-block w-50 float-left theme-text text-right"><a onclick="return nav.go(this);" href="/id<?= $author['uid'] ?>"><?= $author['name'].' '.$author['surname'] ?></a></span>
                </div>
            </div>
        </div>
    </div>
