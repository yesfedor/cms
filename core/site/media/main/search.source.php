<?php $sourceData['content'] = <<<content
<div class="container-fluid mt-5">
    ...
    <div id="mediaWrapperSearchModal" class="d-flex row align-items-center justify-content-center"></div>
</div>
content;
$sourceData['js'] = <<<js
    ...
    if (value.length > 2) {
        $.ajax({
            ...
            success: function (obj) {
                $('#mediaWrapperSearchModal').html('');
                if (obj.code == 404) 
                    $('#mediaWrapperSearchModal').html('<div class="col-12 text-center animated fadeIn"><h4 class="theme-text my-5">Ничего не найдено</h4></div>');
                if (obj.code == 200) 
                    appMediaRender('mediaWrapperSearchModal', obj.content, {fill: 'default', type: false});
            }
        });
    }
    if (el_search.value == '') $('#mediaWrapperSearchModal').html('<div class="col-12 text-center animated fadeIn"><h4 class="theme-text my-5">Начните искать..</h4></div>');
}, urlState='go');
js;
$sourceData['size'] = 'full';
?>